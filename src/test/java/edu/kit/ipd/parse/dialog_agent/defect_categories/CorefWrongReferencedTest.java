...
public class CorefWrongReferencedTest {
	...
	@Test // checks if multiple target entities are returned correctly
	public void testCheckMatch() {
		CorefWrongReferenced cwr = new CorefWrongReferenced();
		IGraph answerGraph = buildGraph(Paths.get("src/main/resources/audiofiles/test/testCoref.flac"));
		List<INode> answerGraphList = answerGraph.getNodes();
		List<INode> targetEntities = new ArrayList<INode>();
		// set target entities
		targetEntities.add(answerGraphList.get(24));
		targetEntities.add(answerGraphList.get(34));
		targetEntities.add(answerGraphList.get(38));
		List<INode> resultList = cwr.checkMatch(targetEntities, answerGraph);
		// compare
